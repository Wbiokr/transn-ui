<template lang='jade'>
  .p-pop-toast
    vDetail
      div(slot='t') toast插件
      div(slot='s') toast弹出提示
      div(slot='s-use') 当项目需要toast提示
      div(slot='s-code') 
        vWrapper
          div.t-com(slot='com')
            p
              input(v-model='msg' placeholder='请输入想要toast内容')
            p
              button.c-p(@click='pop("top")') top弹
              button.c-p(@click='pop("middle")') middle弹
              button.c-p(@click='pop("bottom")') bottom弹
